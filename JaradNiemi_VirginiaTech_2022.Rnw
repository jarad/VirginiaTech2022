...
\end{column}
\begin{column}{0.24\textwidth}
\includegraphics[width=0.7\textwidth]{ben_neo}
\end{column}
\end{columns}
...
\transitionslide{Gaussian Process}
...
Y_i
% = f(X_i, \psi)
= f(X_i) + \epsilon_i, \quad i=1,\ldots,N
\]
...
\[ f \sim \mathcal{GP}\left(m, k \right) \implies Y \sim N(m_x, \Sigma+\tau^2 \mathrm{I}) \]
...
Following \cite{morris2012}, the functional-input Gaussian Process has
